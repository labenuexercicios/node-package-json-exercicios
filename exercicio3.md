# Exercício 3
Dentro da pasta ```exercicios```, crie outra pasta chamada ```node-package-json``` e coloque o arquivo ```node-package-json.js``` dentro dela.<br> Vamos criar um arquivo com nome baseado no jogo que você for desenvolver dentro da pasta ```exercicios/node-package-json```.<br>
